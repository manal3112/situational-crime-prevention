...
for filename in os.listdir(directory):
    ...
    if os.path.isfile(f) and f.lower().endswith('.dat') and f.split('/')[-1].split('.')[0] not in ['ASR94', 'ASR99', 'ASR96']:
      try:
        ...
        fname = "{}.csv".format(f.split('/')[-1].split('.')[0])
...
df = pd.read_csv("ASR02.dat", names = ["Column_1", "Column_2"])
df.drop(columns = ["Column_2"], inplace = True)
...
df["Column_1"].apply(lambda x: x[13:15])[:50], df["Len"][:50]
...
df = pd.read_csv("2020_ASR1MON_NATIONAL_MASTER_FILE.txt", names = ["Column_1", "Column_2"])
...
df[df["Month"] == '00'] # AGENCY_HEADERS

df["Offense_Code"] = df["Column_1"].apply(lambda x: x[17:20])

df[df["Offense_Code"] == '000'] # MONTHLY_HEADERS

df[(df["Month"] != '00') & (df["Offense_Code"] != '000')] # DETAIL_HEADERS
...
df[ df["Monthly_Header"] ]

df[["MSA", "County", "Suburban", "Agency_Name", "State_Name", "Population_for_Agency"]] = None
...
df[["Agency_Name", "Population_for_Agency"]][df["Population_for_Agency"] > 1000000].sort_values("Population_for_Agency")["Agency_Name"]
...
df[(~df["Agency_Header"]) & (~df["Monthly_Header"])]

list(df[(~df["Agency_Header"]) & (~df["Monthly_Header"])]["Column_1"].apply(lambda x: int(x[20:23])).isin([57,58,59,60])).count(True)

df[(~df["Agency_Header"]) & (~df["Monthly_Header"])]["Column_1"].apply(lambda x: int(x[23:26])).unique()
...
df[['Male-Under-10',
 ...
 'Adult-Non-Hispanic',
 'Juvenile-Hispanic',
 'Juvenile-Non-Hispanic']] = None

df.columns

code_column_map = dict(zip(range(1,57), \
['Male-Under-10',
 'Male-10-12',
 'Male-13-14',
 'Male-15',
...
df[(~df["Agency_Header"]) & (~df["Monthly_Header"])].columns
...
final_df.to_csv("Arrests_2020.csv", index = False)

os.chdir('/content/gdrive/My Drive/CSE_545_Project/Arrest_Data/')

import glob
# merging the files
joined_files = os.path.join("/content/gdrive/My Drive/CSE_545_Project/Arrest_Data/", "ASR*.csv")
